...
<body>
	...
	<p class="index-sectionheader">
		<span>当前位置 : </span> <span>首页 - 系统管理 - 编辑功能权限</span> 
	</p>
	<div class="addmes">
		<p class="addclick">
			<span x="1" class="addclick-style">编辑功能权限</span>
		</p>
		<div class="addmes-cnt">
			<form method="POST" id='formproject' onsubmit="return false;">
				<input type="hidden" name="id" id="funid" value="${function.id }">
					<div class="mestr">
					...
					<div class="mestr">
						...
						<div class="lasttd">
							<input id="code" type="text" name="code" value="${function.code }">
						</div>
					</div>
					<div class="mestr">
						<div class="firsttd">访问路径:</div>
						<div class="lasttd">
							<input type="text" name="page" value="${function.page }">
						</div>
					</div>
					<div class="mestr">
						...
						<div class="lasttd">
							<select name="generatemenu">
								<option ${function.generatemenu==0?'selected':'' } value="0" >不生成</option>
								<option ${function.generatemenu==1?'selected':'' } value="1">生成</option>
							</select>
						</div>
					</div>
					<div class="mestr" id="yxj">
						...
						<div class="lasttd">
							<input id="zindex" type="text" name="zindex" value="${function.zindex }"><span id="yxj_ts" style="color:#666666;"></span>
						</div>
					</div>
					<div class="mestr" >
						...
						<div class="lasttd" >
							<select name="pid" id="sel">
								...
								<c:forEach items="${ authFunctionList}" var="fun">
									<option ${function.pid==fun.id?'selected':'' } value="${fun.id }">${fun.name }</option>
								</c:forEach>
							</select>
					...
					<div class="mestr">
						...
						<div class="lasttd">
							<textarea name="description" rows="4" cols="33" value="${function.description }"></textarea>
						</div>
					</div>
			</form>
		</div>
	...
	<script>
			...
			$("#btn_submit").click(function() {
				...
				if(checkHousetype()){
					var name = $("#name").val();
			      	  $.ajax({
			      			type:"GET",//请求类型
			      			url:"${pageContext.request.contextPath }/rest/sys/checkFunctionNameEdit/"+name+"/"+funid,
			      			statusCode : {
			      				200 : function(data){
			      					if(data){
			      				...
			      				500 : function(){layer.msg('网络异常，请稍后再试！', {time:2000});return false;}
			      			}
			      		});
					}
			});
		...
		function submitFrom(){
			...
			$.ajax({
				type : "POST",
				url : "${pageContext.request.contextPath }/rest/sys/saveeidtfun",
// 						data : $("#formproject").serialize(),
				...
				statusCode : {
					201 : function() {
						layer.msg('编辑功能权限成功!', {icon: 6,time:1500},function(){ 
